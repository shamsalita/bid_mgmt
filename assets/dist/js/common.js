...
$(document).on('change', '.type_filter', function(e){
    ...
    if(picker_type == 'day') {
        ...
        $datepicker.datepicker('setDate',($currDate.getMonth() + 1) + '-' + $currDate.getFullYear());
    } else if(picker_type == 'year') {
        $('.end_date_filter').parent().remove();
        $datepicker.datepicker({
            format: "yyyy",
        ...
        $datepickerStart = $('.date_filter');
        $datepickerStart.datepicker({
            format: "dd-mm-yyyy",
            orientation: "bottom",
        });
        $datepickerStart.datepicker('setDate', $currDate);
        ...
        $datepickerEnd = $('.end_date_filter');
        $datepickerEnd.datepicker({
            format: "dd-mm-yyyy",
            orientation: "bottom",
        });
        $datepickerEnd.datepicker('setDate', $currDate);
    }
});
...
$(document).on('click', '.change_status', function(e){
    ...
    if(cnf){
        ...
        if($(this).hasClass('make_project')){
            status = 3;
        }else if($(this).hasClass('make_bid')){
            status = 1;
        }
        var me = $(this);
