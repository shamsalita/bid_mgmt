				...
				<div class="card-body">
					...
					<form>
						<div class="col-md-12 row">
							...
							<div class="col-md-2 form-group">
								<input type="text" class="form-control date_filter" name="date_filter" value="<?php if(isset($_GET['date_filter'])){ echo $_GET['date_filter']; } else { echo date('d-m-Y'); } ?>">
							</div>
					...
					<table id="example2" class="table table-bordered table-hover table-14-fonts">
						<thead>
							<tr>
								...
								<th>Status</th>
								<th>Bid Date</th>
								<?php if($this->session->userdata('user')['role'] == 1 || $this->session->userdata('user')['role'] == 3){ ?>
								<th>Bidder Name</th>
								<?php } ?>
								<th>Lead Date</th>
						...
						<tbody>
							...
							<tr class="<?php echo $tr_class; ?>">
								...
								<td>
									<a href="<?php echo $bid_data['url']; ?>" target="__new"><?php echo $bid_data['url']; ?></a>
								</td>
								<td>
									<?php echo $tech_arr[$bid_data['technology']]; ?>
								</td>
								<td>
									<?php echo $bid_data['rate']; ?>
								</td>
								<td>
									<?php echo $bid_data['country_name']; ?>
								</td>
								<td>
									<?php if ($bid_data['job_type'] == 0) { echo 'Hourly'; } else { echo 'Fixed Price'; }; ?>
								</td>
								<td>
                                    <?php
								...
								<td>
									<?php echo date('d-m-Y', strtotime($bid_data['created'])); ?>
								</td>
								<?php if($this->session->userdata('user')['role'] == 1 || $this->session->userdata('user')['role'] == 3){ ?>
								<td>
								...
								<td>
									<?php
										if($bid_data['lead_date'] != null){
                                            echo date('d-m-Y', strtotime($bid_data['lead_date']));
										}else{
                                            echo '-';
										}
										?>
								...
								<td>
									<?php
										if($bid_data['conversion_date'] != null){
                                            echo date('d-m-Y', strtotime($bid_data['conversion_date']));
										}else{
                                            echo '-';
										}
										?>
								...
								<td>
									...
									<?php
										if($bid_data['status'] == 1){
											if($this->session->userdata('user')['role'] == 1 || $this->session->userdata('user')['role'] == 3){
										?>
									...
									<a class="btn btn-info change_status make_bid" data-bidid="<?php echo $bid_data['id']; ?>">Bid</a>
									<?php
											}
										}elseif($bid_data['status'] == 3){
									?>
									<a class="btn btn-info change_status make_bid" data-bidid="<?php echo $bid_data['id']; ?>">Bid</a>
                              		<a class="btn btn-primary change_status make_lead" data-bidid="<?php echo $bid_data['id']; ?>">Lead</a>
									<?php
										}
									?>
